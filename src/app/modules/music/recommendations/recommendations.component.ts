...
@Component({
  ...
  template: `
    <page
      [isDone]="
        (user$ | async) &&
        (playlistDetails$ | async) &&
        (playlistRecommendations$ | async)
      "
    >
    ...
    try {
      const createPlaylistResponse = await this.playlistService
        .putCreatePlaylist(
          ...
          `Similar playlist to '${this.playlistName}' created by Spotted.`,
        )
        .toPromise();

      // Add all songs
      const recommendationResponse =
        await this.playlistRecommendations$.toPromise();
